...
export function useEvaluationAggregation(): InstructorSummary[] {
  ...
  return useMemo(() => {
    ...
    for (const ev of submittedEvals) {
      if (!byInstructor.has(ev.instructorId))
        byInstructor.set(ev.instructorId, []);
      byInstructor.get(ev.instructorId)!.push(ev);
    }

    const summaries: InstructorSummary[] = Array.from(
      byInstructor.entries(),
    ).map(([instructorId, evals]) => {
      ...
      const classStats = Array.from(byClass.entries()).map(
        ([classId, classEvals]) => {

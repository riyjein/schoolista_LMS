...
import {
  evalRecords as fallbackEvalRecords,
  getEvalRecord,
...
export function useEvaluationValidation(studentId: string) {
  ...
  const canEvaluate = useCallback(
    (classId: string): { can: boolean; reason?: string } => {
      ...
      if (!isEnrolled)
        return { can: false, reason: "Not enrolled in this class." };
      ...
      if (record?.status === "submitted") {
        return {
          can: false,
          reason: "Evaluation already submitted and cannot be changed.",
        };
      }
      return { can: true };
    },
  ...
  const classStatuses = useMemo(
    (): ClassEvalStatus[] =>
      enrolledClasses.map((cls) => ({
        ...
        record: evalRecords.find(
          (r) => r.studentId === studentId && r.classId === cls.id,
        ),
      })),

...
import type { DayAttendance, EnrichedAttendanceRecord, AttendanceSummary } from '../../types/attendance';
...
export function useAttendanceCalendar(studentId: string): UseAttendanceCalendarReturn {
  ...
  const [currentWeekStart, setCurrentWeekStart] = useState(() => startOfWeek(new Date(2024, 7, 5)));
  ...
  const enrichedRecords = useMemo(() => {
    const raw = getRecordsForStudent(studentId);
    const filtered = classFilter ? raw.filter((r) => r.classId === classFilter) : raw;
  ...
  const summary = useMemo<AttendanceSummary>(() => {
    ...
    const excused = enrichedRecords.filter((r) => r.status === 'excused').length;
  ...
  const calendarGrid = useMemo((): CalendarDay[] => {
    ...
    for (let i = startPad; i > 0; i--) {
      ...
      days.push({ date, dayNumber: d.getDate(), isCurrentMonth: false, dayAttendance: dayAttendanceMap.get(date) ?? null });
    }

    for (let day = 1; day <= lastDay.getDate(); day++) {
      const d = new Date(currentYear, currentMonth, day);
      const date = formatDate(d);
      days.push({ date, dayNumber: day, isCurrentMonth: true, dayAttendance: dayAttendanceMap.get(date) ?? null });
    }

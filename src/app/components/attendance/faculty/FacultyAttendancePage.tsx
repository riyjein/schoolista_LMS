        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              2024-2025 · 1st Semester · Monitor student attendance for your
              handled classes.
            </p>
          </div>
                ...
                <div className="rounded-xl border bg-card p-5 shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <div>
                      <h2 className="font-semibold">
                        {att.selectedClass.subjectCode} —{" "}
                        {att.selectedClass.subjectTitle}
                      </h2>
                      <p className="text-sm text-muted-foreground">
                        {att.selectedClass.sectionCode} ·{" "}
                        {att.selectedClass.room} ·{" "}
                        {att.selectedClass.enrolledStudentIds.length} students ·{" "}
                        {att.selectedClass.totalSessions} sessions
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p
                        className={cn(
                          ...
                          att.classSummary.attendanceRate >= 90
                            ? "text-green-700"
                            : att.classSummary.attendanceRate >= 75
                              ? "text-yellow-700"
                              : "text-red-700",
                        )}
                      >
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      {
                        label: "Present",
                        value: att.classSummary.present,
                        color: "text-green-700 bg-green-50 border-green-200",
                      },
                      {
                        label: "Late",
                        value: att.classSummary.late,
                        color: "text-yellow-700 bg-yellow-50 border-yellow-200",
                      },
                      {
                        label: "Absent",
                        value: att.classSummary.absent,
                        color: "text-red-700 bg-red-50 border-red-200",
                      },
                      {
                        label: "Excused",
                        value: att.classSummary.excused,
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="date"
                      value={att.dateFrom}
                      onChange={(e) => att.setDateFrom(e.target.value)}
                    ...
                    <input
                      type="date"
                      value={att.dateTo}
                      onChange={(e) => att.setDateTo(e.target.value)}
                      className="rounded-lg border bg-background px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-primary"
                    />
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {row.isAtRisk && (
                                    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs bg-red-100 text-red-700 border border-red-200">
                                      <AlertTriangle className="h-3 w-3" /> At
                                      Risk
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className="px-5 py-3 bg-muted/20 border-t">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {att.classSessions.map((session) => {
                                    const status =
                                      session.studentStatuses[row.studentId] ??
                                      "no-record";
                                    return (
                                      <div
                                        key={session.date}
                                        className="flex flex-col items-center gap-0.5"
                                        title={`${session.date}: ${STATUS_CONFIG[status].label}`}
                                      >
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-card border-b">
                          <tr>
                            <th className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground min-w-36">
                              Student
                            </th>
                            {att.classSessions.slice(0, 20).map((s) => (
                              <th
                                key={s.date}
                                className="px-1 py-2.5 text-center text-xs font-medium text-muted-foreground whitespace-nowrap"
                              >
                                {s.date.slice(5)}
                              </th>
                            ))}
                      ...
                      {att.classSessions.length > 20 && (
                        <p className="text-xs text-muted-foreground text-center py-2">
                          Showing latest 20 of {att.classSessions.length}{" "}
                          sessions
                        </p>
                      )}
                    </div>
